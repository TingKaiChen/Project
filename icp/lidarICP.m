...
%     if (sza(2)>10) && (szb(2)>10)
    ...
    check = false;
    subplot(2,2,2)
    af = [x(frame,:);y(frame,:);zeros(1,sza(2))];
    bf = [x(preframe,:);y(preframe,:);zeros(1,sza(2))];
    [TR, TT, ER, t, count] = icp(af, bf, iter, 'Matching', 'kDtree');
    if count>0
        totalcount = totalcount+1;
    end
    % Test rotation matrix
    rad = rotm2eul(TR);
    tr = [cos(rad(1)) -sin(rad(1)); sin(rad(1)) cos(rad(1))];
    diff = abs(tr-TR(1:2, 1:2))>1e-10;
    match = match && isequal(diff,[0 0;0 0]);
