...
td = [];
alpha = [];
[R,R1,R2,R3,R4] = deal(eye(3));
[T,T1,T2,T3,T4] = deal(zeros(3,1));
rot = [1 0;0 1];
traj = [0;0];
rot1 = [1 0;0 1];
traj1 = [0;0];
...
for frame=1+step:step:(m/16)
    ...
    [TR, TT, ER] = icp(af, R*bf+repmat(T,1,nzb), iter, 'Matching', 'kDtree', 'WorstRejection', 0.1);
    [TR1, TT1, ER1] = icp(af1, R1*bf1+repmat(T1,1,nzb1), iter, 'Matching', 'kDtree', 'WorstRejection', 0.1);
    [TR2, TT2, ER2] = icp(af2, R2*bf2+repmat(T2,1,nzb2), iter, 'Matching', 'kDtree', 'WorstRejection', 0.1);
    [TR3, TT3, ER3] = icp(af3, R3*bf3+repmat(T3,1,nzb3), iter, 'Matching', 'kDtree', 'WorstRejection', 0.1);
    [TR4, TT4, ER4] = icp(af4, R4*bf4+repmat(T4,1,nzb4), iter, 'Matching', 'kDtree', 'WorstRejection', 0.1);
    eul1 = rotm2eul(TR1*R1);
    eul2 = rotm2eul(TR2*R2);
    eul3 = rotm2eul(TR3*R3);
    eul4 = rotm2eul(TR4*R4);
    eul = [eul; [eul1(1) eul2(1) eul3(1) eul4(1)]];
    td = [td; [norm(TR1*T1+TT1) norm(TR2*T2+TT2) norm(TR3*T3+TT3) norm(TR4*T4+TT4)]];
...
%     alpha = [alpha apa];
    ...
    % Trajectory
    rot = TR(1:2,1:2)'*R(1:2,1:2)'*rot;
    traj = traj-rot*(TR(1:2,1:2)*T(1:2,1)+TT(1:2,1));
    rot1 = TR1(1:2,1:2)'*R1(1:2,1:2)'*rot1;
    traj1 = traj1-rot1*(TR1(1:2,1:2)*T1(1:2,1)+TT1(1:2,1));
    rot2 = TR2(1:2,1:2)'*R2(1:2,1:2)'*rot2;
    traj2 = traj2-rot2*(TR2(1:2,1:2)*T2(1:2,1)+TT2(1:2,1));
    rot3 = TR3(1:2,1:2)'*R3(1:2,1:2)'*rot3;
    traj3 = traj3-rot3*(TR3(1:2,1:2)*T3(1:2,1)+TT3(1:2,1));
    rot4 = TR4(1:2,1:2)'*R4(1:2,1:2)'*rot4;
    traj4 = traj4-rot4*(TR4(1:2,1:2)*T4(1:2,1)+TT4(1:2,1));
