...
v_s1    = vertex;
e_s1    = edges;
load('./pose_graph/LoopClosure/nooc_20170523162617.mat')
v_nooc    = vertex;
e_nooc    = edges;
...
scatter3(v_nooc(:,2),v_nooc(:,3),v_nooc(:,4),PtSize,'r','filled')
% scatter3(v_cycle(:,2),v_cycle(:,3),v_cycle(:,4),PtSize,'r','filled')
axis equal
xlim([min([v_s1(:,2);v_nooc(:,2);v_cycle(:,2)])-25, max([v_s1(:,2);v_nooc(:,2);v_cycle(:,2)])+25])
ylim([min([v_s1(:,3);v_nooc(:,3);v_cycle(:,3)])-25, max([v_s1(:,3);v_nooc(:,3);v_cycle(:,3)])+25])
zlim([min([v_s1(:,4);v_nooc(:,4);v_cycle(:,4)])-15, max([v_s1(:,4);v_nooc(:,4);v_cycle(:,4)])+15])
xlabel('X')
ylabel('Y')
zlabel('Z')
...
se_dist_nooc    = sum((v_nooc(1,2:4)-v_nooc(end,2:4)).^2).^0.5
% se_dist_cycle = sum((v_cycle(1,2:4)-v_cycle(end,2:4)).^2).^0.5

disp(['s1: 	  start: ', num2str(v_s1(1,2:4))])
disp(['s1:    end:   ', num2str(v_s1(end,2:4))])
disp(['nooc: 	  start: ', num2str(v_nooc(1,2:4))])
disp(['nooc:    end:   ', num2str(v_nooc(end,2:4))])
